...
$INCLUDE (ASCII.INC)					;general ASCII characters
...
;Dec2String
;
;Description:           This function takes a 16-bit signed integer along with a 16-bit
;						location.  It will take the integer and create an array of ASCII
;						character representations of the number in decimal, starting at
;						the memory location given as an argument.  The array will begin
;						with a negative sign if the given number is negative and the last
;						value in the array will be the NULL character.  If at any time an
;						error is reached, the function will terminate with the appropriate
;						flag set.  The only thing returned is whether or not an error
;						occurred.  The array this function creates will contain exactly 5
;						integers, meaning if the number is not 5 digits, there will be
;						leading zeroes.  This function assumes the number to be converted
;						is passed in AX and the location to start the array is passed in
;						SI.
; 
; 
;Operation:             This will take the 16-bit number and first check if it is negative
;						or positive by checking whether it is positive or negative.  If it
;						is negative, then '-' will be added to the beginning of the array
;						and our index will be incremented, then the argument will be
;						negated.  Otherwise, it will continue to calculate the decimal
;						string.  Note that '-' is stored as ASCIIdash in ASCII.inc.  Then,
...
;						have been filled in.  Finally, a NULL character will be added to
;						the end of the string.  Once again, this assumes the input number
;						is in AX and the location in SI.
; 
...
MakeDecString:					;initialization
	...
	CLC							;no error yet
	;JMP	CheckSign			;check sign of input

CheckSign:						;procedure to check sign of input
	CMP	BX, 0					;checks if the input is negative
	JL	IsNegative				;go to IsNegative if input < 0
	;JGE	IsPositive			;otherwise, go to IsPositive (works for 0, too)
	
IsPositive:
	MOV BYTE PTR [SI], '+'		;load a plus sign into the array to denote positivity
	INC SI						;increment our array pointer
	;JMP	MainDecLoop			;otherwise, continue

IsNegative:						;handle number if it is negative
	MOV BYTE PTR [SI], '-'		;load '-' into array.
	INC	SI						;increment the array pointer
	NEG BX						;make n positive n <-- n * -1 (BX <-- BX * -1)
	JMP	MainDecLoop				;start the main loop for our now-positive n
...
FindDigit:						;proc. to find the next digit
	...
	MOV	DX, 0					;prepare for dividing n / pwr10
...
EndDecString:					;fill last index with null character
	MOV BYTE PTR [SI], ASCIInull;load NULL into array
	RET
